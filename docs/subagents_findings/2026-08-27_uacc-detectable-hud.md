...
Meeting: Recap, Follow-ups, Email, and Actions pills in fixed top chrome. `computer.meeting_assist` `kind` say/recap/followups/email/actions. `GET /api/meeting?notes=1` returns live notes as untrusted data (no Cortex hop). Copy recap and `GET /api/meeting?recap=1` share the last Recap from main. Copy say and `GET /api/meeting?say=1` share the last Suggest/Say from main. Copy email and `GET /api/meeting?email=1` share the last follow-up email from main. Copy actions and `GET /api/meeting?actions=1` share the last action-item list from main. Meeting LIVE captions are fixed chrome (system audio only, no cursor-follow).
