...
Observe PNG/clipboard: live `computer.observe` captures `captureDisplayCrop` when `screenshot:true` and `driver.clipboardGet` when `clipboard:true`. Loopback `GET /api/observe?screenshot=1&clipboard=1` forwards those flags. `captions:true` / `GET /api/observe?captions=1` returns LIVE speech lines (partials replace the tail) tagged untrusted, not commands. Clipboard is tagged untrusted data. Windows and foreground include x y width height plus center cx cy (GetWindowRect, DIP in main). PATH tokens draw click-through polylines. BOX/RECT tokens frame a control. Overlay now fires for LINE/PATH/BOX-only answers, not just POINT.

Meeting: Recap, Follow-ups, Email, and Actions pills in fixed top chrome. `computer.meeting_assist` `kind` say/recap/followups/email/actions. `GET /api/meeting?notes=1` returns live notes as untrusted data (no Cortex hop). Copy recap and `GET /api/meeting?recap=1` share the last Recap from main. Copy say and `GET /api/meeting?say=1` share the last Suggest/Say from main. Copy email and `GET /api/meeting?email=1` share the last follow-up email from main. Copy actions and `GET /api/meeting?actions=1` share the last action-item list from main. `GET /api/meeting?pack=1` returns those together as one markdown pack. Meeting LIVE captions are fixed chrome (system audio only, no cursor-follow).
