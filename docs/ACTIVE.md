...
| UIA targeting | `electron/netie/uia.js` (Act targeting + teach POINT; InvokePattern on `click:`/`invoke:`; ValuePattern on `fill:`/`type in:`/`set:`; TogglePattern `toggle:`/`check:`/`uncheck:`; ExpandCollapsePattern `expand:`/`collapse:`; no invented coords; no cursor warp) |
