...
| Coworker desks | `electron/netie/coworker-desks.js` (`teach` measured POINT plus Tab/Enter; loopback host sticky cue bar with a fixed Ask field plus `/teach` Back/Got it Ask the next step; `meeting` Heard names/orgs; `/meeting` and `/today` Ask chips file inbox/Word drafts, never Act; inbox greets those names; Word notes titled from Heard; `today` plate is commitments only, not the say-this dump; meeting spawn files inbox+Word+security drafts, never Act) |
