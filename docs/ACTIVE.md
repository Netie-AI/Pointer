...
| HUD | `electron/hud.html`, `hud.js`, `hud.css`; click-through POINT/LINE/PATH/BOX teach layer; Report a problem `#bugReportBtn` copies local diagnostics (`electron/netie/bug-report.js`) |
...
| Excel safe API | `electron/netie/excel-coworker.js` (`buildXlsxChart` OOXML chart-bearing `.xlsx`; Act still needs Cortex; no Windows-MCP / P-08) |
