...
| UACC skills + loopback detect | `electron/netie/uacc.js`; MCP `computer.status` (mode, hotkeys, STT, LLM, privacy) / `observe` / `act` (mode switch) / `scribe` / `meeting_assist`; loopback `/api/computer` `/api/observe` `/api/scribe` `/api/meeting` `/api/tools` on `127.0.0.1:18010`; observe optional screenshot + clipboard; HUD `#privacy-chip` |
