...
| Coordinator + first-party MCP ABI | `electron/netie/coordinator.js`, `mcp-abi.js`, `host-serve.js`; pages in `host/` (`/` `/today` `/meeting` `/lanes` `/skills` `/workspace`); public Worker `workers/netie-host.js` |
