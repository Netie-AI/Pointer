...
| UIA targeting | `electron/netie/uia.js` (Act targeting + teach POINT; named `click:` tries InvokePattern with no cursor warp, miss falls back to SendInput; `invoke:` is Invoke only; no invented coords) |
...
| Driver (SendInput) | `electron/netie/driver.js` (`uia_invoke` is InvokePattern, not mouse) |
