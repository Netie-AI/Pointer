...
| UIA targeting | `electron/netie/uia.js` (Act targeting + teach POINT; named `fill:` / `type in:` try ValuePattern with no cursor warp, miss falls back to click then type; `set:` is ValuePattern only; no invented coords) |
...
| Driver (SendInput) | `electron/netie/driver.js` (`uia_set` is ValuePattern, not keystrokes) |
