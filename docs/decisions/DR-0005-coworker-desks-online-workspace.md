...
each BOX (later ink dashed). `/teach` This screen paints field / button
faces at those measured percents. A click-through laptop overlay paints that
...
and shows Click/Type in on `/teach` (not `Next: 1 of 3`). `/teach` chrome
is teach-only; `/meeting` and home keep the live answer. Loopback
...
not a markdown dump. Session markdown stays copy-only. The desks catalog
lives on `/workspace`. Download packet is loopback GET `/api/session.zip` (session
...
answer. Opening a Word file paints Notes. Opening unsent mail paints a compose
window whose To is a Heard name or not sent (never invent, never send).
Opening a review paints Needs you (never approve). Loopback `/document`
`/inbox` `/security` paint the same windows. Loopback `/workspace` This computer dock Run POSTs `/api/workspace/exec`
