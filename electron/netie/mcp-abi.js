...
const CATALOG = Object.freeze([
  ...
  },
  {
    name: "computer.act",
    description:
      "Gated OS actions. instruction plans via recipes then type:/click:/click window:/invoke:/toggle:/check:/uncheck:/expand:/collapse:/focus:/open:/deliver:/replace:/wait/scroll/doubleclick/rightclick/hover. click: Save tries UIA InvokePattern first (no cursor warp); miss falls back to SendInput. invoke: Save is Invoke only. toggle: Remember me uses TogglePattern. expand: Documents uses ExpandCollapsePattern. Chain local verbs with then: focus: notepad then type: hello or invoke: Save then type: hello or toggle: Word wrap then type: hello. use Claude opens Claude Code while the 5-hour window is open; use Cursor when that limit is used. Clicks and launches need approved true. mode alone switches Agent/General/Transcribe/Scribe/Meeting like the tray (no Cortex).",
    inputSchema: {
      type: "object",
      properties: {
