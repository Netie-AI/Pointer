...
const CATALOG = Object.freeze([
  ...
  },
  {
    name: "computer.act",
    description:
      "Gated OS actions. instruction plans via recipes then type:/click:/click window:/toggle:/check:/uncheck:/expand:/collapse:/focus:/open:/deliver:/replace:/wait/scroll/doubleclick/rightclick/hover. toggle: Remember me uses UIA TogglePattern (no cursor). expand: Documents uses ExpandCollapsePattern (no cursor). Chain local verbs with then: focus: notepad then type: hello or toggle: Word wrap then type: hello or expand: Documents then type: hello. use Claude opens Claude Code while the 5-hour window is open; use Cursor when that limit is used. Clicks and launches need approved true. mode alone switches Agent/General/Transcribe/Scribe/Meeting like the tray (no Cortex).",
    inputSchema: {
      type: "object",
      properties: {
