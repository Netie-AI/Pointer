...
class NetieEcosystem {
  ...
  async _llmPlan({ safeInstruction, dataUrl, hotContext = "" }) {
    const system = [
      ...
      "  {\"type\": one of observe|read|hover|scroll|movecursor|click|doubleclick|rightclick|type|press|focus_hwnd|word_docx_write|word_docx_append|word_from_clipboard|excel_xlsx_chart|clipboard_paste,",
      "   \"target\": short human description,",
      "   \"value\": text to type or key name (type/press), or the document body (word_docx_write / word_docx_append), or the chart series (excel_xlsx_chart: 'Q1 10, Q2 20'),",
      ...
      "When the user wants an Excel chart, emit excel_xlsx_chart with value set to the series (for example Q1 10, Q2 20). Do not drive the Excel ribbon or Windows-MCP. Omit path so the workbook lands in the sanctioned folder.",
