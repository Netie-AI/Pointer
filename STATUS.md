...
2. **Coworker desks (DR-0005).** Loopback sticky chrome Ask bar plus
   `/meeting` and `/today` chips file inbox/Word/security drafts (never
   send, never a .docx, never Act). `/teach` Back/Got it. Today plate is
   commitments only. Public catalog empty. P-06 parked.
