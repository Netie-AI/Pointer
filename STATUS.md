...
0. **Gates that can now fail** - CI exists (`.github/workflows/ci.yml`, two
...
1. **The status pill's Open button never worked** - `hud:openPath` had a handler
...
2. **EPIC-P02 boundary implemented on branch** - one path resolver
...
3. **P05/P06 gaps closed** - attachments actually attach (#23), app names are
...
4. **Chrome pinned against the rendered HUD** (#22) - writing it found the LIVE
