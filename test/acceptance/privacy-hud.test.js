...
(async () => {
  ...
  const tests = [
    ...
    T("the HUD stays tight: CSP unweakened, no floating companion", async () => {
      ...
      assert.ok(html.includes('id="btn-report"'), "Report a problem stays in fixed top chrome");
      assert.ok(html.includes('id="report-panel"'), "report panel is HUD chrome, not a settings page");
