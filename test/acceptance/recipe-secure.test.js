...
async function run() {
  ...
  const tests = [
    T("secureBeforeAct helper exists and fail-closes", () => {
      ...
      assert.ok(/exportMeetingActions/.test(main), "Copy actions must export the last action items in main");
      ...
      assert.ok(/action === "actions"/.test(main), "hud:meetingNotes actions must not take renderer text");
