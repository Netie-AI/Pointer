...
(async () => {
  ...
  const tests = [
    ...
    T("the overlay is not a companion and cannot eat clicks", async () => {
      ...
      assert.ok(/id="walk-asked"/.test(walk) && /id="walk-live"/.test(walk) && /paintLiveChrome/.test(walk), "overlay chrome keeps They asked / You / Live captions after Ask or Save");
      assert.ok(/if \(saved \|\| wasSaved\) hideWalkDock\(\)/.test(walk), "after Save the leftover Live answer dock drops");
